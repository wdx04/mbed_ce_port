 ...
 */
Adafruit_SGP30::Adafruit_SGP30(I2C *i2c)
{
  i2c_dev = i2c;
}
 ...
 *  @param  initSensor
 *          Optional parameter to prevent IAQinit to be called. Used for Deep
 *          Sleep.
 *  @return True if SGP30 found on I2C, False if something went wrong!
 */
bool Adafruit_SGP30::init() {
 ...
 *           wrong!
 */
bool Adafruit_SGP30::softReset(void) {
  uint8_t command[2];
  command[0] = 0x00;
  command[1] = 0x06;
 ...
 * wrong!
 */
bool Adafruit_SGP30::IAQmeasureRaw(void) {
  uint8_t command[2];
  command[0] = 0x20;
  command[1] = 0x50;
  uint16_t reply[2];
 ...
 *           wrong!
 */
bool Adafruit_SGP30::getIAQBaseline(uint16_t *eco2_base,
                                       uint16_t *tvoc_base) {
  uint8_t command[2];
  command[0] = 0x20;
  command[1] = 0x15;
 ...
 *          wrong!
 */
bool Adafruit_SGP30::setHumidity(uint32_t absolute_humidity) {
...
bool Adafruit_SGP30::readWordFromCommand(uint8_t command[],
    uint8_t commandLength, uint16_t delayms, uint16_t *readdata,
    uint8_t readlen)
{
  if (i2c_dev->write((SGP30_I2CADDR_DEFAULT << 1) & 0xFE, reinterpret_cast<const char *>(command), commandLength) != 0) {
    return false;
  }
  ...
  if (i2c_dev->read((SGP30_I2CADDR_DEFAULT << 1) | 0x01, reinterpret_cast<char*>(replybuffer), replylen) != 0) {
    return false;
  }
...
#ifdef I2C_DEBUG
    printf("CRC calced: 0x%02X vs. 0x%02X\n", crc, replybuffer[i * 3 + 2]);
#endif
