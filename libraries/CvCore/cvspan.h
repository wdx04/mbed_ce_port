...
struct span_capacity {
    static constexpr bool value = E == cv::dynamic_extent || E == N;
};
...
struct span_is_span<cv::span<T, E> > {
...
struct span_implicit {
    static constexpr bool value = E == cv::dynamic_extent ||
        N != cv::dynamic_extent;
};

template<class T, std::size_t E, class U, std::size_t N>
struct span_copyable {
    static constexpr bool value = (N == cv::dynamic_extent ||
        span_capacity<E, N>::value) && span_convertible<U, T>::value;
};

template<std::size_t E, std::size_t O>
struct span_sub {
    static constexpr std::size_t value = E == cv::dynamic_extent ?
        cv::dynamic_extent : E - O;
};
...
template<class T>
struct span_store<T, cv::dynamic_extent> {
...
template<class T>
struct span_bytes<T, cv::dynamic_extent> {
    static constexpr std::size_t value = cv::dynamic_extent;
};
