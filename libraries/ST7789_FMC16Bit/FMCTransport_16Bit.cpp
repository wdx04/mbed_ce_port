...
FMCTransport_16Bit::FMCTransport_16Bit(int subbank_no, int address_no)
{
...
#endif
      TFT_LCD_BASE += (uint32_t)((2U << address_no) - 2);
      init(subbank_no, address_no);
  }
}

bool FMCTransport_16Bit::is_valid()
{
    return TFT_LCD != nullptr;
}

void FMCTransport_16Bit::init(unsigned int subbank_no, unsigned int address_no)
{
    ...
    TFTSRAM_Handler.Init.MemoryDataWidth=FMC_NORSRAM_MEM_BUS_WIDTH_16;
...
void FMCTransport_16Bit::write_register(uint8_t regval)
{
    TFT_LCD->LCD_REG=regval;
}

void FMCTransport_16Bit::write_data(uint16_t data)
{
    TFT_LCD->LCD_RAM=data;
}

void FMCTransport_16Bit::write_data(const uint16_t *data, size_t count)
{
...
uint16_t FMCTransport_16Bit::read_data(void)
{
    volatile uint16_t ram=TFT_LCD->LCD_RAM;
    return ram;
}

volatile uint16_t* FMCTransport_16Bit::get_register_pointer()
{
    return &TFT_LCD->LCD_REG;
}

volatile uint16_t* FMCTransport_16Bit::get_data_pointer()
{
